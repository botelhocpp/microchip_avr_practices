...
void timer_init(timer_id id, timer_mode mode, timer_clock_select clock_select) {
    ...
    // Clear WGM
    HWREG(TIMER_TCCRA_REGS[id]) &= ~(0b11 << 0); // WGMn1:WGMn0
    HWREG(TIMER_TCCRB_REGS[id]) &= ~(0b11 << 3); // WGMn3:WGMn2

    HWREG(TIMER_TCCRA_REGS[id]) |= ((mode_mask & 0b11) << 0);   // WGMn1:WGMn0
...
void timer_set_clock(timer_id id, timer_clock_select clock_select) {
    ...
    HWREG(TIMER_TCCRB_REGS[id]) &= ~(0b111 << 0);
    HWREG(TIMER_TCCRB_REGS[id]) |= (clock_select_value << 0);
}
...
void timer_enable_interrupt(timer_id id, timer_interrupt interrupt) {
    HWREG(TIMER_TIMSK_REGS[id]) |= (1 << interrupt);
}
